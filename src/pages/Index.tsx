...
const AtlantisWaterPark = () => {
    ...
    // Estados individuais do formulário
    ...
    useEffect(() => {
        ...
        // Clientes mockados iniciais (opcional)
        const clientesIniciais: Cliente[] = [
            {
                ...
                documentos: [
                    { id: 1, numero: '123.456.789-00', tipo: 'CPF', dataExpedicao: new Date() }
                ],
                dependentes: []
            }
        ];
    ...
    const adicionarDependente = async (dependente: Omit<Cliente, 'id' | 'dataCadastro' | 'dependentes'>, titularId: number) => {
        ...
        setClientes(prev => [...prev, novoDependente]);

        // Adicionar à lista de dependentes do titular
        setClientes(prev => prev.map(c =>
            c.id === titularId
                ? { ...c, dependentes: [...c.dependentes, novoDependente] }
                : c
        ));

        alert(`Dependente ${dependente.nome} vinculado a ${titular.nome}!`);
    };
    ...
    const realizarCheckIn = async (clienteIdParam: number, acomodacaoIdParam: number) => {
        ...
        if (cliente && acomodacao && acomodacao.disponivel) {
            ...
            setAcomodacoes(prev => prev.map(a =>
                a.id === acomodacaoIdParam
                    ? { ...a, disponivel: false }
                    : a
            ));

            alert(`Check-in realizado! ${cliente.nome} - Quarto ${novaHospedagem.numeroQuarto}`);
        }
    };
    ...
    const realizarCheckOut = async (hospedagemId: number) => {
        ...
        // Atualizar hospedagem como inativa
        setHospedagens(prev => prev.map(h =>
            h.id === hospedagemId
                ? { ...h, ativa: false, dataCheckOut: new Date() }
                : h
        ));

        // Liberar acomodação
        setAcomodacoes(prev => prev.map(a =>
            a.id === hospedagem.acomodacao.id
                ? { ...a, disponivel: true }
                : a
        ));
    ...
    const removerCliente = async (clienteIdParam: number) => {
        ...
        if (confirm(`Tem certeza que deseja remover ${cliente.nome}? Esta ação não pode ser desfeita.`)) {
            setClientes(prev => prev.filter(c => c.id !== clienteIdParam && c.titular?.id !== clienteIdParam));
